...
)  # Papu Python va a crear la carpeta si no existe. Gracias Wenner, sin vos esto no era posible
                    ...
                    for (row, col), cell in tabla.get_celld().items():
                        if row == 0:
                            ...
                            if val == "Libre":
                                ...
                                cell.set_facecolor(
                                    "#e9ecef"
                                )  # Gris clarito (fuera de horario)
                                cell.set_text_props(color="black")

                    ruta_imagen = f"static/agenda_{num_telefono}.png"
                    plt.savefig(
                        ruta_imagen, bbox_inches="tight", dpi=150, transparent=False
                    )
                    plt.close(fig)
