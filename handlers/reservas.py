...
from utils.helpers import (
    ...
    extraer_hora,
    obtener_inicio_semana_reservas,
)
...
async def manejar_reservas(
    ...
    if msg == "1" and estado_actual == "inicio":
        ...
        try:
            ...
            for fila in datos_deudores[1:]:
                ...
                if (
                    len(fila) >= 5
                    and fila[0] == num_telefono
                    and fila[4].strip().lower() == "pendiente"
                ):
                    es_deudor = True
        ...
        if msg in ["1", "2", "3", "4"]:
            ...
            for i in range(inicio_rango, fin_rango):
                ...
                if fecha_str in excepciones:
                    ...
                    if exc["tipo"] == "cerrado":
                        ...
                        ini, fin = exc["horas"].split("-")[0].strip().zfill(5), exc[
                            "horas"
                        ].split("-")[1].strip().zfill(5)
                ...
                if hay_bloque:
                    dias_disponibles.append(
                        f"{nombre_dia.capitalize()} ({fecha_dt.strftime('%d/%m')})"
                    )
            ...
            if dias_disponibles:
                ...
                if cantidad_turnos > 1:
                    res_text += f"\n*(Como sacaste {cantidad_turnos} turnos, podés mandar todos los nombres juntos separados por coma. Ej: Lunes 16:00 Juan Pérez 1, Lucas Gómez 2)*"
                else:
                    res_text += "\n*(Ej: Lunes 16:00 Juan Pérez 1)*"
    ...
    if estado_actual == "eligiendo_dia" and "cancelar" not in msg:
        ...
        dia_det = next(
            (d for d in mapa.keys() if quitar_tildes(d) in quitar_tildes(msg)), None
        )
        cantidad_turnos = sesiones[num_telefono].get("cantidad_turnos", 1)
        ...
        if dia_det:
            ...
            if h_des:
                if h_des in inicios_validos:
                    ...
                    for p in partes:
                        if not dia_removido and quitar_tildes(dia_det) in quitar_tildes(
                            p
                        ):
                            dia_removido = True
                            continue
                    ...
                    if texto_restante and tiene_numero:
                        ...
                        sesiones[num_telefono]["lista_servicios"] = [
                            ...
                            for i, f in enumerate(datos_servicios[1:], start=1)
